...
export default function EventDetailPage() {
  ...
  const handlePurchase = async () => {
    ...
    try {
      const buyerWalletAddress = localStorage.getItem('buyerWalletAddress');
      if (!buyerWalletAddress) {
        setError('No se encontró la dirección de la billetera del comprador');
        setPurchasing(false);
        return;
      }
      
      const zonesForApi = getSelectedZones();
      
      await ticketsApi.purchase(event.id, totalTickets, zonesForApi, buyerWalletAddress);
      
      router.push('/tickets');
    } catch (err: any) {
      setError(err.message || 'Error al comprar tickets');
    } finally {
      setPurchasing(false);
    }
  };
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="pt-4 border-t border-gray-200">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <p className="text-sm text-gray-500 font-mono">ESTIMATED_GAS_FEE</p>
                      </div>
                      <p className="text-sm text-gray-600">
                        Transaction will include minimal gas fees (~0.0001 ETH)
                      </p>
                    </div>
                  </div>
                </div>
